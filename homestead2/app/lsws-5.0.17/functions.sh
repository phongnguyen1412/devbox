...
installation()
{
    ...
    util_mkdir "$SDIR_OWN" $DIR_MOD admin bin docs fcgi-bin lib logs admin/logs add-ons share  admin/fcgi-bin admin/html.$VERSION admin/misc admin/misc/apimport
    ...
    util_cpfile "$SDIR_OWN" $EXEC_MOD admin/misc/create_admin_keypair.sh admin/misc/awstats_install.sh admin/misc/apimport.sh
    ...
    util_cpdir "$SDIR_OWN" $DOC_MOD admin/misc/apimport
...
EOF
    ...
    SUCC=0
    while [ $SUCC -eq "0" ];  do
    printf "%s" "Would you like to import Apache configuration [y/N]? "

    read DOIMPORT
    if [ "x$DOIMPORT" = 'x' ]; then
        DOIMPORT=n
    fi
    if [ `expr "$DOIMPORT" : '[Yy]'` -gt 0 ]; then
        printf "%s" "Path to your Apache configuration file [Enter to skip]: "
        read APCONFPATH
        if [ "x$APCONFPATH" != "x" ]; then
            if [ -f "$APCONFPATH" ]; then
                $LSWS_HOME/admin/misc/apimport.sh "$APCONFPATH"
                SUCC=1
            else
                echo "[ERROR] Cannot find the configuration file:$APCONFPATH."
            fi
        else
            SUCC=1
        fi
